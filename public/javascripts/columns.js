...
function setParentEvent(dom) {
    $(dom).on("touchstart touchmove touchend MSPointerDown MSPointerMove MSPointerUp", function (event) {
...
function loadingImgDiv(dom, data) {
    ...
    $.each(data, function (key, value) {
        ...
        setImgTouchs("#columns-img-" + key, "#columns-img-bar", value.url);
    });
    loadNoImg(left, dom);
}
//加载空图片
...
//设置图点击事件
function setImgTouchs(dom, bar, url) {
//    alert("sss");
    $(dom).on("touchend MSPointerUp", function (event) {
        $(bar).trigger("touchend MSPointerUp");
        $("#columns-parent").css({"-webkit-transform":"translate3d(0px,0px,0px)",
            "-ms-transform":"translate3d(0px,0px,0px)"});
        $("#columns-matte").css("opacity", "0");
